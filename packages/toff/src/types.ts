/**
 * Tree of Failures (ToF) - Core Type Definitions
 *
 * This file defines the fundamental types used in the ToF system.
 * Each type plays a specific role in building and managing the tree
 * of tasks, attempts, and their outcomes.
 */
...
export type Action = {
   ...
   * Examples:
   * - For shell: { command: string }
   * - For files: { filename: string, content: string }
   * - For http: { url: string, method: string, body?: object }
   */
